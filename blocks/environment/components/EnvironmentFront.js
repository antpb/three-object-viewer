...
function ChatBox(props) {
	...
	//create a state for the input message
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			const channel = "wordpress";
				...
				console.log("its getting in here now");
				let newFeed = [];
				...
				// remove the last 30 messages from the feed
				newFeed = newFeed.slice(Math.max(newFeed.length - 30, 0));
	
	
				//convert currentFeed to a string from json
				// stringify the newFeed 
				let feedString = JSON.stringify(newFeed);
				console.log("Feed string:", feedString);
				...
				const postData = {
					Input: {
						Input: ("What is on the timeline? : " + feedString),
						Speaker: "speaker",
						Agent: agent,
						Client: client,
						ChannelID: channelId,
				...
				const response = await fetch('/wp-json/wp/v2/callAlchemy', {
					...
					body: JSON.stringify(postData)
						...
						if(thisMessage?.model === "gpt-4-0314"){
							...
							props.setMessages([...props.messages, inputMessage, formattedMessage]);
	...
	const handleSubmit = async (event) => {
		// set the input message to the value of the input
	...
	input.value = '';
	  ...
	  const agent = props.name;
	  const channel = "wordpress";
	if (value === "What's on the skyline?") {
		await handleTimeline();
		// turn the props.feedItems object into a json string
	}
	try {
		if ( value === "What's on the skyline?") {
			return
	...
	const agent = new BskyAgent({
		...
		persistSession: (evt, sess) => {
			...
			fetch('/wp-json/your_namespace/v1/unified_session/', {
				...
				headers: {
					'Content-Type': 'application/json',
					'X-WP-Nonce': props?.userData?.nonce, // Make sure to enqueue the wp-api script in your WordPress plugin or theme
				},
